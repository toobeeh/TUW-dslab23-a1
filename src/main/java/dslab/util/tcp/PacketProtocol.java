...
public abstract class PacketProtocol {
    ...
    interface PacketHandler<TPacket extends Packet> {
        Packet handle(TPacket packet) throws PacketHandleException, ProtocolCloseException;
    }

    private final HashMap<String, PacketHandler> packetHandlers = new HashMap<>();
    private final HashMap<String, PacketFactory> packetFactories = new HashMap<>();
    ...
    public Packet handle(String data) throws ProtocolCloseException {
        ...
        try {
            ...
            return new ErrorPacket().withMessage(e.getResponseString());
        }
        ...
        try {
            ...
            if(result == null) result = packet.getResponsePacket();
        }
    ...
    private Class<? extends Packet> validateCommandPacketHandler(Method candidate){
        ...
        if(paramTypes.length == 0 || !Packet.class.isAssignableFrom(paramTypes[0])){
    ...
    private PacketHandler getPacketHandler(Method method){
        return data -> {
            try {
                ...
                return result == null ? null : (Packet) result;
